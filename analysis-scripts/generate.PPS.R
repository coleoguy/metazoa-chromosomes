...
plants <- c("asteraceae", "fabaceae", "brassicaceae", "orchidaceae", "lilaceae", "solanaceae", "pteridophyta")
...
for(m in 1:length(clades)){
  ...
  mcmc_path <- paste0("../../results/exponential-prior-full-model/mentor_results/",clade, ".p.res.csv") # Assuming MCMC output is in current dir
  tree_file <- clade
  ...
  for(i in 1:n_sims){
    ...
    # we will set the root to be equal to the median haploid count
